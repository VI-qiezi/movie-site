<template>
    <div class="popular-comments">
        <ul>
            <li class="comment-item" v-for="pop in popularComment.slice(0,a)" :key="pop._id">
                <a :href="getNewHref(pop._id)" class="item-imgs picHover" :title="pop.title" target="_blank">
                    <img :src="pop.coverPicture" :alt="pop.title" class="hvr-glow">
                </a>
                <div class="item-txt">
                    <a :href="getNewHref(pop._id)" class="txt-title" target="_blank">{{pop.title}}</a>
                    <p class="txt-abstract">
                        {{pop.coverAbstract}}
                    </p>
                    <div class="txt-column">
                        <span class="author">{{pop.author}}</span>
                        <span class="grade">评分：</span>
                        <span class="com-grade">{{pop.grade}} 星</span>
                        <span class="time">{{pop.publicTime}}</span>
                        <div class="action">
        ...
        <div class="btn-more" v-if="a<popularComment.length" @click="loadMore">更多评论</div>
...
export default {
  ...
  methods: {
    getPopularComments() {
      var vm = this;
      this.axios
        .get("/api/comments/getPopularComments")
        .then(function(res) {
          if (res.data.code == 200) {
            vm.popularComment = res.data.popularComment;
